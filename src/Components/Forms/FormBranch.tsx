...
const FormBranch: React.FC<FormBranchProps> = ({ branchId, onBack, mode }) => {
  ...
  const form = useForm<BranchFormValues>({
    resolver: zodResolver(branchSchema) as unknown as Resolver<
      BranchFormValues,
      unknown
    >,
    defaultValues: {
  ...
  const { data: branchData, isLoading: isLoadingBranch } =
    useGet<BranchResponse>({
      endpoint: branchId ? `/api/branch/${branchId}/` : "",
      queryKey: ["branch", branchId],
      enabled: !!branchId && mode === "edit",
    });

  const { mutate: createBranch, isLoading: isCreating } = useMutate<
  ...
  const { mutate: updateBranch, isLoading: isUpdating } = useMutate<
    Record<string, unknown>
  >({
    endpoint: branchId ? `/api/branch/${branchId}/` : "",
    method: "patch",
  ...
  useEffect(() => {
    if (branchData && mode === "edit") {
      ...
      // هنا المشكلة - يجب تعيين الـ ID وليس القيمة النصية
      ...
      if (branchData.currencies) {
        setValue(
          "currencies",
          branchData.currencies.map((curr) => ({
      ...
      if (branchData.payment_methods) {
        setValue(
          "payment_methods",
          branchData.payment_methods.map((pm) => ({
  ...
  const onSubmit: SubmitHandler<BranchFormValues> = (data) => {
    ...
    if (data.currencies && data.currencies.length > 0) {
      data.currencies.forEach((currency, index) => {
        ...
        formData.append(
          `currencies[${index}]exchange_rate`,
          currency.exchange_rate.toString(),
        );
        formData.append(
          `currencies[${index}]default`,
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (key.includes("[") && key.includes("]")) {
        ...
        if (match) {
          ...
          const array = requestData[arrayName] as Array<
            Record<string, unknown>
          >;
          ...
          array[Number(index)][fieldName] = value;
        }
      } else {
            ...
            <div>
              ...
              >
                ...
                {paymentMethodsData?.results?.map((method) => (
                  <option key={method.id} value={method.country}>
                    {method.country}
                  </option>
                ))}
              </select>
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      {...register(`currencies.${index}.currency`, {
                        valueAsNumber: true,
                      })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    >
                  ...
                  <div>
                    ...
                    <input
                      {...register(`currencies.${index}.exchange_rate`, {
                        valueAsNumber: true,
                      })}
                      type="number"
                      step="0.0001"
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    ...
                    <select
                      {...register(`payment_methods.${index}.payment_method`, {
                        valueAsNumber: true,
                      })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    >
