...
export default function ContactBaseInfo({
  ...
  const {
    ...
    error,
    refetch,
  } = useGet<BaseContactData>({
    endpoint: `/api/${title}/${contactId}/`,
    queryKey: [title, contactId],
  });

  const { mutate: updateContact, isLoading: isUpdating } = useMutate<
  ...
  useEffect(() => {
    if (contactData && isEditing) {
      const changesExist = Object.keys(formData).some(
        (key) => formData[key] !== contactData[key],
      );
  ...
  if (isLoading)
    return <div className="p-4">{t("contactInfo.messages.loading")}</div>;
  if (error)
    return (
      <div className="p-4 text-red-500">
        {t("contactInfo.messages.errorLoading")}
      </div>
    );
    ...
    name: {
      label:
        contactType === "supplier"
          ? t("contactInfo.supplierName")
          : t("contactInfo.customerName"),
      type: "text",
      required: true,
    },
    phone: {
      ...
      <div className="w-full flex flex-col h-full">
        <div className="relative mb-8">
          ...
          <div className="relative flex flex-col items-center pt-2">
            ...
            <h2 className="text-lg py-2 font-bold text-gray-900">
              {contact?.name || t("contactInfo.messages.noName")}
            </h2>
          </div>
        </div>
        ...
        <div className="shadow flex flex-col w-full bg-white rounded-lg">
          <div className="w-full flex items-center justify-between bg-bgtitle p-2 rounded-t-lg">
            <h2 className="text-lg py-2 font-medium text-gray-900">
              {t("contactInfo.basicInformation")}
            </h2>
            <div className="flex items-center gap-3">
              {isEditing && contactType === "supplier" ? (
                ...
                >
                  {contact?.is_active
                    ? t("contactInfo.status.active")
                    : t("contactInfo.status.inactive")}
                </button>
              ) : (
                contactType === "supplier" && (
                  <div
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                {contact?.modified_at && (
                  <div>
                    <label className="text-sm text-gray-500">
                      {t("contactInfo.lastModified")}
                    </label>
                    <p className="text-gray-900">
                      {new Date(contact.modified_at).toLocaleDateString()}
                    </p>
                  </div>
                )}
        ...
        {isEditing ? (
          ...
          <div className="flex justify-end space-x-4 mt-6">
            <button
              ...
              className="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {t("contactInfo.editInfo", {
                type: t(`contactInfo.${contactType}`),
              })}
            </button>
          </div>
        )}
