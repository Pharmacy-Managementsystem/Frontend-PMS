...
const UserInfo: React.FC<UserInfoProps> = ({
  ...
  const { data: rolesResponse } = useGet<{
  ...
  useEffect(() => {
    if (userData) {
      const branchIds =
        userData.user_branches?.map((branch) => branch.id.toString()) || [];
  ...
  const handleSave = async () => {
    ...
    if (userData) {
      ...
      if (editData.role && editData.role !== userData.role_name) {
        const selectedRole = rolesResponse?.results.find(
          (role) => role.name === editData.role,
        );
        if (selectedRole) {
            ...
            <div className="relative mb-8">
              ...
              <div className="relative flex flex-col items-center pt-2">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  ID: <span className="capitalize">{userData.role_name}</span> #
                  {userData.id}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 h-full">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex justify-between items-center pb-4 mb-5 bg-gray-100 rounded-lg border-gray-200">
                <h3 className="text-base font-semibold p-3 text-gray-800">
                  {t("userInfo.userDetails")}
                </h3>
              </div>
              <div className="flex flex-col">
                {isEditing && editMode === "full" ? (
                  <>
                    {["username", "phone_number", "email", "address"].map(
                      (field) =>
                        canEditField(field) && (
                          <div key={field} className="ps-3 pb-3">
                            <label className="block text-base text-gray-500 mb-2 capitalize">
                              {t(`userInfo.${field}`)}
                            </label>
                            <input
                              id={field}
                              ...
                              value={
                                editData[
                                  field as keyof typeof editData
                                ] as string
                              }
                              onChange={(e) =>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex justify-between items-center pb-4 mb-5 bg-gray-100 rounded-lg border-gray-200">
                <h3 className="text-base font-semibold text-gray-800 p-3 capitalize">
                  {t("userInfo.assignedPermissions")} {userData.role_name}
                </h3>
              </div>
