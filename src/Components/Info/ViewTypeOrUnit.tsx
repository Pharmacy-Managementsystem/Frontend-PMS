...
const ViewTypeOrUnit: React.FC<ViewTypeOrUnitProps> = ({ itemId, itemType, onBack }) => {
  ...
  const getParentOptions = () => {
    ...
    return allItemsResponse.results.filter(item => 
      item.id !== itemId && // exclude self
      (!itemData || item.level === undefined || itemData.level === undefined || item.level < itemData.level)
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm">
        <div className="p-6 bg-white rounded-lg shadow-lg text-center">
          <p className="text-red-500 mb-4">{t('viewTypeOrUnit.errorLoading', { itemType: t(`viewTypeOrUnit.${itemType}`) })}</p>
          <button
            onClick={onBack}
  ...
  const displayName = itemType === 'unit' ? t('viewTypeOrUnit.unit') : t('viewTypeOrUnit.type');
            ...
            {!isEditing ? (
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  ...
                  <span>
                    {updateMutation.isPending ? t('viewTypeOrUnit.saving') : t('viewTypeOrUnit.saveChanges')}
                  </span>
                </button>
              </div>
            )}
                ...
                <div>
                  ...
                  {isEditing ? (
                    <input
                      ...
                      placeholder={t('viewTypeOrUnit.enterName', { displayName: displayName.toLowerCase() })}
                    />
                  ) : (
                    <p className="px-3 py-2 bg-gray-50 rounded-md">{itemData.name}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    title='Parent'
                      ...
                      onChange={(e) => handleInputChange('parent', e.target.value ? parseInt(e.target.value) : null)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
