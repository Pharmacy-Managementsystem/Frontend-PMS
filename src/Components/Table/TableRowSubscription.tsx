        ...
        <td key={col} className={`py-4 px-6 text-sm text-gray-900 ${isRTL ? 'text-right' : 'text-left'}`}>
          {col === 'Status' ? (
            <span className={`inline-flex items-center gap-1 px-3 py-1 text-xs font-medium rounded-full ${statusColors[String(data[col])] || 'bg-gray-100 text-gray-500'}`}>
              ...
              {String(data[col]) === 'Active' ? t('table.status.active') :
               String(data[col]) === 'Expired' ? t('table.status.expired') :
               String(data[col]) === 'Cancelled' ? t('table.status.cancelled') :
               String(data[col]) === 'Suspended' ? t('table.status.suspended') :
               String(data[col]) === 'Pending' ? t('table.status.pending') :
               data[col]}
            </span>
          ) : (
