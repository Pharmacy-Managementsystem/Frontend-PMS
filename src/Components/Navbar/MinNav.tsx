...
const dropdownContent = {
  ...
  inventory: ["products", "stockPoint", "stockTransfers"],
...
export default function MinNav({
  ...
  const filteredItems = items.filter((item) => {
    if (navItems === "management") {
      if (decodedToken?.is_superuser) {
        return (
          item === "businessManagement" ||
          item === "packageManagement" ||
          item === "subscriptionsManagement"
        );
      }
      return (
        item === "userManagement" ||
        item === "branchesManagement" ||
        item === "roles"
      );
    }
    return true;
  ...
  const getActiveTab = () => {
    if (navItems === "management") {
      if (decodedToken?.is_superuser) {
        if (
          filteredItems.includes(activeTab as (typeof filteredItems)[number])
        ) {
          return activeTab;
        } else {
  ...
  useEffect(() => {
    if (decodedToken && navItems === "management") {
      ...
      const isAccessingRestrictedTab = isSuperuser
        ...
        : activeTab === "businessManagement" ||
          activeTab === "packageManagement";

      if (isAccessingRestrictedTab) {
        const appropriateTab = isSuperuser
          ? "businessManagement"
          : "userManagement";
    ...
    <div key={language}>
      <div className="top-16 left-0 sticky right-0 z-40 mt-8">
        <div className="max-w-screen-xl mx-auto">
          <div
            className={`flex flex-wrap md-nowrap items-center justify-center h-16 ${isRTL ? "space-x-reverse space-x-12" : "space-x-12"}`}
          >
            {filteredItems.map((item, index) => (
              ...
              >
                <span
                  className={`text-sm font-medium pb-3 px-6 ${
