...
export default function BusinessManage() {
  ...
  const tableData =
    businessResponse?.results?.map((business) => ({
      ...
      [t("businessManagement.subscriptionStatus")]: (
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusBadgeClass(business.subscription_status)}`}
        >
      ...
      [t("businessManagement.contactNumber")]:
        business.contact_number || t("contactInfo.messages.notAvailable"),
    ...
    <>
      {businessAction ? (
        <div>
          <FormBusiness
            mode={
              businessAction === t("businessManagement.addNewBusiness")
                ? "add"
                : "edit"
            }
            businessId={
        ...
        <div>
          <TableHeaderSearch
            ...
            onAddClick={() =>
              setBusinessAction(t("businessManagement.addNewBusiness"))
            }
          />
