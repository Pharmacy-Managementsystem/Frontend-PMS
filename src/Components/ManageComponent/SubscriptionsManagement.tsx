...
export default function SubscriptionsManagement() {
  ...
  const [status, setStatus] = useState<
    "all" | "active" | "expired" | "cancelled" | "suspended" | "pending"
  >("all");
  ...
  const { data: packagesData } = useGet<{
    results?: PackageItem[];
    data?: PackageItem[];
  }>({
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Pagination
              currentPage={page}
              totalItems={
                (data as { count?: number })?.count ?? tableData.length
              }
              itemsPerPage={pageSize}
              onPageChange={(p) => setPage(p)}
              hasNext={Boolean((data as { next?: string | null })?.next)}
              hasPrevious={Boolean(
                (data as { previous?: string | null })?.previous,
              )}
            />
          </>
        )}
